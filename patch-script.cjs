...
const toggleSpeakingStart = "const toggleSpeaking = () => {\n    if (!speechAPI) return;\n    if (isGloballyPlaying) {\n      speechAPI.cancel();\n      updateGlobalState(false);\n    } else {";

// Let's replace the whole `else { ... }` block
const elseBlockEnd = "updateGlobalState(true);\n    }";

const oldCodeStart = code.indexOf(toggleSpeakingStart) + toggleSpeakingStart.length;
const oldCodeEnd = code.indexOf(elseBlockEnd, oldCodeStart) + elseBlockEnd.length;

if (oldCodeStart > toggleSpeakingStart.length && oldCodeEnd > oldCodeStart) {
  code = code.substring(0, oldCodeStart) + "\n" + ttsReplace + "\n    }" + code.substring(oldCodeEnd);
} else {
  console.log("Could not find toggleSpeaking block to replace.");
}
