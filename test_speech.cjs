...
// I will patch the speakWhenReady function.
const oldCode = `      // Bulletproof voice loading and playback
...
const newCode = `      // Bulletproof voice loading and playback
      const speakWhenReady = () => {
          ...
          const hasTargetLang = () => {
             ...
             return speechAPI.getVoices().some(v => v.lang.toLowerCase().includes(language.toLowerCase()));
          };

          // If voices aren't loaded yet or missing the target language, poll for them (up to 3 seconds)
          if (voices.length === 0 || !hasTargetLang()) {
              let attempts = 0;
              const checkVoices = setInterval(() => {
                  voices = speechAPI.getVoices();
                  if (hasTargetLang() || attempts > 30) {
                      clearInterval(checkVoices);
                      executeSpeak();
                  }
                  attempts++;
